...
Pulls a tiny static ``au-runtime.json.gz`` from this repo's GitHub Release on
first tool call so a dead endpoint can be repointed (edit runtime/au-runtime.json
...
Opt-out: set ``AU_ELI_RUNTIME_URL=""`` to disable the fetch.
"""
...
MANIFEST_URL = os.environ.get(
    "AU_ELI_RUNTIME_URL",
    "https://github.com/matematicsolutions/au-eli-mcp/releases/latest/download/au-runtime.json.gz",
)
_CACHE_NAME = "au-runtime.json"
_TIMEOUT = 15
_USER_AGENT = "au-eli-mcp-runtime (+https://github.com/matematicsolutions/au-eli-mcp)"

_runtime: dict[str, Any] | None = None


def _cache_dir() -> Path:
    env = os.environ.get("AU_ELI_CACHE_DIR")
    if env:
        return Path(env).expanduser()
    return Path.home() / ".matematic" / "cache" / "au-eli"
